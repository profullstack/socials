...
__export(src_exports, {
  ...
  runLoginAutomation: () => runLoginAutomation
});
...
function getBrowser(name) {
  switch (name) {
    case "firefox":
      return import_playwright.firefox;
    case "webkit":
      return import_playwright.webkit;
    default:
      return import_playwright.chromium;
  }
}
async function runLoginAutomation(options) {
  ...
  try {
    ...
    if (options.storageStateOut) {
      await (0, import_promises.mkdir)((0, import_node_path.dirname)(options.storageStateOut), { recursive: true });
      const state = await context.storageState();
      await (0, import_promises.writeFile)(options.storageStateOut, JSON.stringify(state, null, 2), {
        mode: 384
      });
    }
    return {
...
async function loadSecret(ref, label) {
  ...
  if (ref.file) {
    const value = (await (0, import_promises2.readFile)(ref.file, "utf8")).trim();
